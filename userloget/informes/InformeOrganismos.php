...
//table header        
    ...
    $this->Cell(300,10,'Listado Sueldos Por Organismos',100,100,'C');//Titulo
...
if  (empty($_POST['txtOrganismo'])){$codigo=0;}else{$codigo=$_POST['txtOrganismo'];}
//------------------------------------------------------------------------------      
$pdf->AddPage('L', 'Legal');
$pdf->AliasNbPages();
...
$consulta=pg_exec($conectate,"SELECT row_number()over (partition by 0 order by max(SAL.sal_cod)) as lineas,
		    ...
		    ,max(lin.lin_des) as nrolinea
                    ...
                    and EXTRACT(YEAR FROM sal_fecha)= EXTRACT(YEAR FROM now()) group by FUN.fun_cod order by nrolinea,FUN.fun_nom");
