...
$result=mysql_query("SELECT trabajo.tra_cod AS codigo,CONCAT(funcionario.fun_nom,' ',funcionario.fun_ape) AS nombre,actividad.act_des AS actividad,
					trabajo_det.trad_obs AS obs,users.nombre AS nom,dependencia_imp.dei_item,dependencia_imp.dei_fec,impresora.imp_cod,impresora.imp_mod,impresora.imp_npa,trabajo.tra_fec 
				FROM trabajo,trabajo_i,funcionario,actividad,trabajo_det,users,impresora,dependencia_imp 
				WHERE trabajo.fun_cod=funcionario.fun_cod AND trabajo_det.act_cod=actividad.act_cod AND trabajo_det.tra_cod=trabajo.tra_cod 
					AND trabajo.id=users.id AND trabajo.tra_cod=trabajo_i.tra_cod AND dependencia_imp.imp_cod=trabajo_i.imp_cod AND impresora.imp_cod=dependencia_imp.imp_cod 
					AND impresora.imp_cod=trabajo_i.imp_cod AND dependencia_imp.dei_est=1 AND trabajo_det.tra_cod=".$trab." 
...
$txttitle = "\n\n LISTADO DE ACTIVIDADES REALIZADAS A LA IMPRESORA "."\n"; //Titulo general
$pdf->ezText($txttitle, 12,array('justification'=>'center'));
$datas = array(array('uno'=>'Impresora: <u>'.$feyus["imp_mod"].'</u>','medio'=>'','dos'=>'Asignado desde: <u>'.$feyus["dei_fec"].'</u>'));
...
$datas2= array(array('uno'=>'Encargado: <u>'.$feyus["nombre"].'</u>','medio'=>'','dos'=>'Nro. Patrimonial: <u>'.$feyus["imp_npa"].'</u>'));
