...
if($nroci==0)
	{
			...
			alert("No hay datos para mostrar");
			 window.location="http://localhost/app/phpsueldos/userloget/informes/FrmReciboFunc.php";
			 </script>';
			}
...
$consulta=pg_exec($conectate,"SELECT row_number()over (partition by 0 order by max(SAL.sal_cod)) as lineas,
                    ...
                    and EXTRACT(YEAR FROM sal_fecha)= EXTRACT(YEAR FROM now()) group by SAL.sal_fecha");
