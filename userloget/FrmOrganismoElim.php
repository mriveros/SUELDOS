...
include './funciones.php';
 if  (empty($_COOKIE["var2"])){
    ...
    $query = "delete from organismo where org_cod= ".$codigo.";"; 
                  ...
                  // include './funciones.php';
                   //hace una conexion local
                    ...
                    if ($row = pg_fetch_array($result)){ 
                       ...
                       echo " <caption>Eliminar Organismos (Presione Ctrl+F para buscar)</caption>";
                       echo "<th>Línea</th><th>Código</th><th>Descripción</th><th>Eliminar</th> \n"; 
                       do { 
                       echo "<tr><td>".$row["lineas"]."</td><td>".$row["org_cod"]."</td><td>".$row["org_des"]."</td><td><span class='editar' value='".$row["org_cod"]."' , OnClick='Eliminar(".$row["org_cod"].");'>Borrar</span></td></tr> \n"; 
