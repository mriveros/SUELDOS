...
?>
    ...
    <body>
        ...
        <div class='clearfix'></div>
                 ...
                 <?php
                     conexionlocal();
                      $result = pg_query("SELECT 'XXXX' as lineas, 'XXXXXXX'  as nombres,'XXXXXXXX' as sal_fecha, 'XXXXXXXX' as tde_des,sum(SAL.sal_ips)as ips,sum(SAL.sal_aus)as ausencia,sum(SAL.sal_pyt)as permiso,sum(SAL.sal_jud)as judicial,
                        ...
                        LEFT OUTER JOIN tipo_descuento TIPDES
                        on TIPDES.tde_cod=DES.tde_cod
                        where SAL.fun_cod=FUN.fun_cod and FUN.fun_fuente='10' and EXTRACT(MONTH FROM sal_fecha)= EXTRACT(MONTH FROM now())
                        and EXTRACT(YEAR FROM sal_fecha)= EXTRACT(YEAR FROM now())");
                       //total de sueldo bruto
                      $bruto=pg_query("select sum(c.cat_nom)as sueldobruto from salario s,categoria c,categoria_detalle cd ,funcionario fun where  fun.fun_cod=s.fun_cod and fun.fun_fuente='10'
                          and EXTRACT(MONTH FROM sal_fecha)= EXTRACT(MONTH FROM now())
                    and EXTRACT(YEAR FROM sal_fecha)= EXTRACT(YEAR FROM now())
                    and c.cat_cod=cd.cat_cod and cd.fun_cod=s.fun_cod");
                      $row00=pg_fetch_array($bruto);
                      //total de sueldo neto
                      $neto=pg_query("select sum(s.sal_neto)as saldoneto from salario s, funcionario fun where  fun.fun_cod=s.fun_cod and fun.fun_fuente='10' 
                          and EXTRACT(MONTH FROM sal_fecha)= EXTRACT(MONTH FROM now())
                    and EXTRACT(YEAR FROM sal_fecha)= EXTRACT(YEAR FROM now())");
                      $row1=pg_fetch_array($neto);
                      //total de IPS
                      $totalIPS=pg_query("select sum(s.sal_ips) as sal_ips from salario s,funcionario fun where s.fun_cod=fun.fun_cod and fun.fun_fuente='10' 
                          and EXTRACT(MONTH FROM sal_fecha)= EXTRACT(MONTH FROM now())
                    and EXTRACT(YEAR FROM sal_fecha)= EXTRACT(YEAR FROM now())");
                      $row2=pg_fetch_array($totalIPS);
                      //total ausencia
                      $totalAus=pg_query("select sum(s.sal_aus) as ausencia from salario s,funcionario fun where s.fun_cod=fun.fun_cod and fun.fun_fuente='10' 
                          and EXTRACT(MONTH FROM sal_fecha)= EXTRACT(MONTH FROM now())
                    and EXTRACT(YEAR FROM sal_fecha)= EXTRACT(YEAR FROM now())");
                      $row3=pg_fetch_array($totalAus);
                      //para permiso y llegadas tardias
                      $totalPermiso=pg_query("select sum(s.sal_pyt) as permiso from salario s,funcionario fun where s.fun_cod=fun.fun_cod and fun.fun_fuente='10' 
                          and EXTRACT(MONTH FROM sal_fecha)= EXTRACT(MONTH FROM now())
                    and EXTRACT(YEAR FROM sal_fecha)= EXTRACT(YEAR FROM now())");
                      $row4=pg_fetch_array($totalPermiso);
                      //para descuento judicialez
                      $totalJudiciales=pg_query("select sum(s.sal_jud) as judiciales from salario s,funcionario fun where s.fun_cod=fun.fun_cod and fun.fun_fuente='10' 
                          and EXTRACT(MONTH FROM sal_fecha)= EXTRACT(MONTH FROM now())
                          and EXTRACT(YEAR FROM sal_fecha)= EXTRACT(YEAR FROM now())");
                      $row5=pg_fetch_array($totalJudiciales);
                      //para descuento ASO
                      $totalASO=pg_query("select sum(s.sal_aso) as aso from salario s,funcionario fun where s.fun_cod=fun.fun_cod and fun.fun_fuente='10' 
                          and EXTRACT(MONTH FROM sal_fecha)= EXTRACT(MONTH FROM now())
                    and EXTRACT(YEAR FROM sal_fecha)= EXTRACT(YEAR FROM now())");
                      $row6=pg_fetch_array($totalASO);
                      //para descuento reposo
                      $totalReposo=pg_query("select sum(s.sal_rep) as reposo from salario s,funcionario fun where s.fun_cod=fun.fun_cod and fun.fun_fuente='10' 
                          and EXTRACT(MONTH FROM sal_fecha)= EXTRACT(MONTH FROM now())
                    and EXTRACT(YEAR FROM sal_fecha)= EXTRACT(YEAR FROM now())");
                      $row7=pg_fetch_array($totalReposo);
                      //para total descuentos(otros)
                      $totalOdescuentos=pg_query("select sum(d.ode_mon) as descuentos from descuento d, funcionario fun,salario sal where sal.sal_cod=d.sal_cod and fun.fun_cod=sal.fun_cod and fun.fun_fuente='10' 
                          and EXTRACT(MONTH FROM sal_fecha)= EXTRACT(MONTH FROM now())
                    and EXTRACT(YEAR FROM sal_fecha)= EXTRACT(YEAR FROM now())");
          ...
          <a href="http://localhost/app/phpsueldos/userloget/informes/InformeSueldoF10.php"><img src="img/boton_pdf_descarga.png" title="Enviar a PDF" style="position:center; bottom: 50px; left: 6%;" /></a>
          <a href="http://localhost/app/phpsueldos/userloget/excel/InformeExcelF10.php"><img src="img/btnexcel.jpg" title="Enviar a Excel" style="position:absolute; bottom: 50px; left: 95%;" /></a>
          </div> 
</body>
