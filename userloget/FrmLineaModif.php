...
include './funciones.php';
 conexionlocal();
$result = pg_query("SELECT lin_cod,lin_des FROM linea where lin_cod= ".$codigo); 
...
?>
    ...
    <body>
        ...
        <div class='clearfix'></div>
                    ...
                    <div id="twitter">
                        <form autocomplete="off" action="clases/ClsLinea.php?nuevo=2" method="post" onClik="submit" > 
                                                        <div id='name' class='outerDiv'>
							<input type="hidden" name="txtcodigo" value="<?php echo $row['lin_cod'];?>" required  /> 
							<div class='message' id='nameDiv'>  </div>
                                                            </div>
						<div id='name' class='outerDiv'>
                                                    
							<label for="txtLinea">Descripción:</label> 
							<input type="text" name="txtLinea" value="<?php echo $row['lin_des'];?>" required  /> 
                   ...
                   //hace una conexion local
                    ...
                    if ($row = pg_fetch_array($result)){ 
                       echo "<table style='margin: 6 auto;' heigth=100% width=80% bgcolor='white' border='5' bordercolor='black' cellspacing='3' cellpadding='3' onclick='Refrescar();'> \n"; 
                         echo " <caption>Modificar Lineas (Presione Ctrl+F para buscar)</caption>";
                       echo "<th>Línea</th><th>Código</th><th>Descripción</th><th>Modificar</th> \n"; 
                       do { 
                       echo "<tr><td>".$row["lineas"]."</td><td>".$row["lin_cod"]."</td><td>".$row["lin_des"]."</td><td><span class='editar' value='".$row["lin_cod"]."' , OnClick='Modificar(".$row["lin_cod"].");'>Editar</span></td></tr> \n"; 
